...
def carac(array)
    puts array.count{|handle| handle.length == 6}
end
...
# -------------
def maj(array)
    puts array.count{|handle| handle.upcase == true}
end
...
# -------------
def tri_taille(array)
    puts array.sort_by{|handle| handle.length}
end
...
# -------------
def pos(array)
    puts array.index{|handle| handle == "@epenser"}
end
...
def repartition(array)
    repartition = array.group_by{|handle| handle.length}
    # Initialisation du array values
    tab_values = []
    tab_keys = []
    repartition.each do |keys, values|
        valeurs = values.count
        # On remplit nos 2 array vides
        tab_values << valeurs
        tab_keys << keys
    end
